...
import { IntermediateStep } from "@/components/ui/intermediate-step";
      ...
      {props.messages.map((m) => {
        ...
        return (
          <ChatMessageBubble
            ...
            message={m}
            aiEmoji="🤖"
            sources={props.sourcesForMessages[m.id] || []}
          />
        );
      })}
    </div>
...
export function ChatWindow(props: {
    ...
    onResponse(response) {
      ...
      if (sources.length && messageIndexHeader !== null) {
        setSourcesForMessages({
          ...sourcesForMessages,
          [messageIndexHeader]: sources.map((source: Source) => ({
  ...
  async function sendMessage(e: FormEvent<HTMLFormElement>) {
    ...
    for (let i = 0; i < toolCallMessages.length; i += 2) {
      ...
      // Skip if either message is undefined or toolMessage has no content
      if (!aiMessage || !toolMessage || toolMessage.content === undefined) {
        continue;
      }
